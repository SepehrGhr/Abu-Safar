            ...
            <div className="bg-black/20 backdrop-blur-xl rounded-2xl shadow-2xl p-4 border border-white/20">
                    ...
                    <div className="flex bg-black/20 p-1 rounded-full border border-white/10">
                        {tabs.map((tab) => (
                            <button
                                ...
                                className={`relative flex items-center space-x-2 px-4 py-2 font-semibold text-sm rounded-full transition-colors duration-300 ${activeTab === tab.id ? 'text-brand-dark' : 'text-white/80 hover:text-white'}`}
                            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4 items-center">
                    <div className="relative lg:col-span-3">
                        ...
                        <input type="text" placeholder="From" className="w-full bg-white/10 backdrop-blur-md text-white placeholder:text-white/50 border-2 border-transparent focus:border-brand-primary focus:ring-0 rounded-xl py-3 pl-12 pr-4"/>
                    </div>
                    ...
                    <div className="relative lg:col-span-3">
                        <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-white/50" />
                        <input type="text" placeholder="To" className="w-full bg-white/10 backdrop-blur-md text-white placeholder:text-white/50 border-2 border-transparent focus:border-brand-primary focus:ring-0 rounded-xl py-3 pl-12 pr-4"/>
                    </div>

                    <div className="relative lg:col-span-4 grid grid-cols-2 gap-2">
                        <input type="date" className="w-full bg-white/10 backdrop-blur-md text-white placeholder:text-white/50 border-2 border-transparent focus:border-brand-primary focus:ring-0 rounded-xl py-3 px-4"/>
                        <div className={`transition-opacity duration-300 ${tripType === 'one-way' ? 'opacity-50' : 'opacity-100'}`}>
                           <input
                                type="date"
                                placeholder="Return"
                                disabled={tripType === 'one-way'}
                                className="w-full bg-white/10 backdrop-blur-md text-white placeholder:text-white/50 border-2 border-transparent focus:border-brand-primary focus:ring-0 rounded-xl py-3 px-4 disabled:cursor-not-allowed"
                            />
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-1">
                        <motion.button
                            ...
                            whileTap={{ scale: 0.95 }}
                            className="w-full h-[52px] flex items-center justify-center bg-brand-primary text-white font-bold rounded-xl hover:bg-brand-secondary transition-colors shadow-lg hover:shadow-brand-primary/50 group"
                        >
                            <Search size={24}/>
                        </motion.button>
                    </div>
