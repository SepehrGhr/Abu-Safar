...
func main() {
	...
	roundTripPairs := seedRoundTrips(db)
	...
	seedRoundTripReservations(db, roundTripPairs)
	processPayments(db)
	seedReports(db)
}
...
func processPayments(db *sql.DB) {
	...
	successCount := int(float64(len(pendingPaymentIDs)) * 1.0)
...
// seedAdditionalServicesForTrip adds a random number of services to a single trip.
func seedAdditionalServicesForTrip(db *sql.DB, tripID int64) {
    services := []string{"Internet", "Food service", "Bed"}
    numServices := gofakeit.Number(0, 3)
    gofakeit.ShuffleStrings(services)
    for i := 0; i < numServices; i++ {
        // Using ON CONFLICT DO NOTHING to avoid errors on duplicate service insertion
        _, err := db.Exec(`INSERT INTO additional_services (trip_id, service_type) VALUES ($1, $2) ON CONFLICT DO NOTHING`, tripID, services[i])
        if err != nil {
            log.Printf("Error inserting additional service for trip %d: %v", tripID, err)
        }
    }
}

// seedTransportDetailsForTrip adds specific vehicle details for a single trip.
func seedTransportDetailsForTrip(db *sql.DB, tripID int64, tripType string) {
    airportNames := []string{"JFK International", "LAX", "Heathrow", "Charles de Gaulle", "Dubai Intl"}
    var err error
    switch tripType {
    case "TRAIN":
...
type RoundTripPair struct {
    OutgoingTripID int64
    ReturnTripID   int64
}

// seedRoundTrips creates pairs of trips that are logical round trips.
func seedRoundTrips(db *sql.DB) []RoundTripPair {
    // Fetch necessary data from DB
    var locationIDs []int
    rows, _ := db.Query("SELECT location_id FROM location_details")
    for rows.Next() {
        var id int
        rows.Scan(&id)
        locationIDs = append(locationIDs, id)
    ...
    companyRows, _ := db.Query("SELECT company_id, vehicle_type FROM companies")
    companiesByType := make(map[string][]int)
    for companyRows.Next() {
        var id int
        var vehicleType string
        companyRows.Scan(&id, &vehicleType)
        companiesByType[vehicleType] = append(companiesByType[vehicleType], id)
    }

    tripTypes := []string{"TRAIN", "BUS", "FLIGHT"}
    var createdRoundTrips []RoundTripPair

    fmt.Printf("🌱 Seeding %d round trips...\n", numRoundTrips)
    for i := 0; i < numRoundTrips; i++ {
        // 1. Setup trip details
        rand.Shuffle(len(locationIDs), func(i, j int) { locationIDs[i], locationIDs[j] = locationIDs[j], locationIDs[i] })
        originLocationID, destinationLocationID := locationIDs[0], locationIDs[1]
        selectedType := tripTypes[rand.Intn(len(tripTypes))]
        companyIDs := companiesByType[selectedType]
        if len(companyIDs) == 0 {
            continue
        ...
        // 2. Create the outgoing trip
        outgoingDeparture := time.Now().Add(time.Duration(gofakeit.Number(24, 1500)) * time.Hour)
        outgoingArrival := outgoingDeparture.Add(time.Duration(gofakeit.Number(2, 24)) * time.Hour)
        outgoingCompanyID := companyIDs[rand.Intn(len(companyIDs))]

        var outgoingTripID int64
        err := db.QueryRow(`INSERT INTO trips (origin_location_id, destination_location_id, departure_timestamp, arrival_timestamp, company_id, stop_count, total_capacity) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING trip_id`,
            originLocationID, destinationLocationID, outgoingDeparture, outgoingArrival, outgoingCompanyID, rand.Intn(3), rand.Intn(51)+50).Scan(&outgoingTripID)
        if err != nil {
            log.Printf("Error inserting outgoing trip: %v\n", err)
            continue
        }

        // 3. Create the return trip
        layoverDuration := time.Duration(gofakeit.Number(24, 96)) * time.Hour // 1 to 4 day layover
        returnDeparture := outgoingArrival.Add(layoverDuration)
        returnArrival := returnDeparture.Add(time.Duration(gofakeit.Number(2, 24)) * time.Hour)
        returnCompanyID := companyIDs[rand.Intn(len(companyIDs))]

        var returnTripID int64
        err = db.QueryRow(`INSERT INTO trips (origin_location_id, destination_location_id, departure_timestamp, arrival_timestamp, company_id, stop_count, total_capacity) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING trip_id`,
            destinationLocationID, originLocationID, returnDeparture, returnArrival, returnCompanyID, rand.Intn(3), rand.Intn(51)+50).Scan(&returnTripID)
        if err != nil {
            log.Printf("Error inserting return trip: %v\n", err)
            continue
        }

        // 4. Seed details for both trips using the helpers
        seedTicketsForTrip(db, outgoingTripID, selectedType)
        seedTransportDetailsForTrip(db, outgoingTripID, selectedType)
        seedAdditionalServicesForTrip(db, outgoingTripID)

        seedTicketsForTrip(db, returnTripID, selectedType)
        seedTransportDetailsForTrip(db, returnTripID, selectedType)
        seedAdditionalServicesForTrip(db, returnTripID)

        createdRoundTrips = append(createdRoundTrips, RoundTripPair{OutgoingTripID: outgoingTripID, ReturnTripID: returnTripID})
    }

    fmt.Printf("✅ Seeded %d round trip pairs.\n", len(createdRoundTrips))
    return createdRoundTrips
}

// seedRoundTripReservations creates reservations that book both legs of a round trip.
func seedRoundTripReservations(db *sql.DB, roundTrips []RoundTripPair) {
    if len(roundTrips) == 0 {
        fmt.Println("⚠️ No round trips available to make reservations for.")
        return
    }

    successfulReservations := 0
    fmt.Printf("🌱 Seeding %d round-trip reservations...\n", numRoundTripReservations)
    for i := 0; i < numRoundTripReservations; i++ {
        // Select a random round trip pair
        pair := roundTrips[rand.Intn(len(roundTrips))]

        // For simplicity, we are not checking capacity here, but in a real scenario you would.
        // Your trigger 'increment_reserved_capacity' will raise an exception if capacity is exceeded.

        var reservationID int64
        // Create a single reservation record with is_round_trip = TRUE
        err := db.QueryRow(`INSERT INTO reservations (user_id, is_round_trip) VALUES ($1, TRUE) RETURNING reservation_id`, gofakeit.Number(1, numUsers)).Scan(&reservationID)
        if err != nil {
            log.Printf("❌ Failed to insert round-trip reservation: %v", err)
            continue
        }

        // Create the first ticket link for the outgoing trip
        age := gofakeit.RandomString([]string{"ADULT", "CHILD"})
        _, err = db.Exec(`INSERT INTO ticket_reservation (trip_id, age, reservation_id, seat_number) VALUES ($1, $2, $3, $4)`, pair.OutgoingTripID, age, reservationID, gofakeit.Number(1, 150))
        if err != nil {
            log.Printf("❌ Failed to insert outgoing ticket_reservation for round-trip %d: %v", reservationID, err)
            // In a real app, you'd want to delete the reservation record here.
            continue
        }

        // Create the second ticket link for the return trip
        _, err = db.Exec(`INSERT INTO ticket_reservation (trip_id, age, reservation_id, seat_number) VALUES ($1, $2, $3, $4)`, pair.ReturnTripID, age, reservationID, gofakeit.Number(1, 150))
        if err != nil {
            log.Printf("❌ Failed to insert return ticket_reservation for round-trip %d: %v", reservationID, err)
            continue
        }

        successfulReservations++
    }
    fmt.Printf("✅ Seeded %d true round-trip reservations.\n", successfulReservations)
}
